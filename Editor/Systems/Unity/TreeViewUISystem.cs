...
namespace uFrame.Editor.Unity
{
    public class TreeViewUISystem : DiagramPlugin, IDrawTreeView, IQueryDesignerWindowModalContent
    {
        ...
        public IPlatformDrawer PlatformDrawer
        {
            get
            {
                return _platformDrawer ?? (_platformDrawer = InvertApplication.Container.Resolve<IPlatformDrawer>());
            }
        ...
        public IItem[] TreeData
        {
            get
            {
                return _treeData ?? (_treeData = WorkspaceService.CurrentWorkspace.Graphs.OfType<IItem>().ToArray());
            }
            set { _treeData = value; }
        }
...
//    }
            ...
            Action<Vector2, IItem> itemRightClicked = null)
        {
            ...
            var position = bounds;
            var usedRect =
                position.WithWidth(Math.Max(bounds.width,
                    PlatformDrawer.CalculateTextSize(viewModel.LargestString, CachedStyles.ListItemTitleStyle).x +
                    5*viewModel.MaxIdentLevel))
                    .Pad(0, 0, 15, 0)
                    .WithHeight(ItemHeight*viewModel.TreeData.Count(s => s.Visible));
            ...
            if (!hasItems)
            {
                ...
                PlatformDrawer.DrawLabel(textRect, "No Items Found", CachedStyles.WizardSubBoxTitleStyle,
